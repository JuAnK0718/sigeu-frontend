...
import { User, Lock, ArrowRight, LogOut, AlertTriangle, MapPin, CheckCircle, Trash2, Activity, Shield, Flame, Hospital, Navigation, Camera, Loader2, Eye, X } from 'lucide-react'
...
function App() {
  ...
  const [emergencyForm, setEmergencyForm] = useState({ 
    title: '', description: '', location: '', type: 'ACCIDENT', targetEntity: 'POLICIA', image: '' 
  })
  ...
  const handleSend = async (e) => {
    ...
    const res = await fetch(`${API}/emergencies`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(emergencyForm)
    })
    if (res.ok) { 
      alert('Reporte enviado con éxito. La entidad ya puede ver la información y la foto.'); 
      setEmergencyForm({ title: '', description: '', location: '', type: 'ACCIDENT', targetEntity: 'POLICIA', image: '' }) 
  ...
  // 🔥 LA FUNCIÓN DE INTELIGENCIA ARTIFICIAL 100% REAL
  const handleImageCapture = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      // Mensaje mientras esperamos que Railway y Google respondan
      ...
      try {
        // Petición REAL a tu servidor Python
        ...
        // Extraemos la respuesta que nos da tu código de Python (Gemini)
        const data = await response.json();

        // Escribimos el resultado real en el formulario
        setEmergencyForm(prev => ({ 
          ...prev, 
          description: `[ANÁLISIS DE IA]: ${data.descripcion}` 
        }));
            ...
            <form onSubmit={handleSend} className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <label htmlFor="cameraInput" className="bg-blue-100 text-blue-700 px-4 py-2 rounded-lg text-xs font-black cursor-pointer flex items-center gap-2 hover:bg-blue-200 uppercase tracking-tighter transition-all">
                    {isAnalyzing ? <Loader2 className="animate-spin" size={14}/> : <Camera size={14}/>} {isAnalyzing ? 'Procesando...' : 'Analizar Escena'}
                  </label>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                <select className="w-full p-4 bg-white border border-slate-200 rounded-xl font-bold text-slate-800 outline-none" value={emergencyForm.targetEntity} onChange={e => setEmergencyForm({...emergencyForm, targetEntity: e.target.value})}>
                  <option value="POLICIA">Policía Nacional</option><option value="BOMBEROS">Bomberos</option><option value="HOSPITAL">Hospital</option>
                </select>
                <button type="submit" className="w-full bg-[#ff0000] text-white p-4 rounded-xl font-black text-lg shadow-lg shadow-red-500/30 flex items-center justify-center gap-2 transform active:scale-95 transition-all uppercase italic">ENVIAR REPORTE</button>
              </div>
            </form>
          </div>
        ) : (
